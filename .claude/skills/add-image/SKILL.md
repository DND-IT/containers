...
Every image lives in `images/<name>/` with exactly two files: `Dockerfile` and `metadata.yaml`. Copy an existing image directory as a template if there is one, otherwise work through the steps below from scratch.
...
3. Open a PR — CI builds both arches on native runners and runs the metadata `test`. Merging to `main` publishes `ghcr.io/dnd-it/<name>` with tags `X.Y.Z`, `X.Y`, `X`, `latest`, plus SBOM and provenance attestations.
